...
B.repeatedly(2000, ['log']).onValue(function () {
...
function validSession(id, remotePort, remoteAddress) {
  var session = sessions[id];
  return session && session.port === remotePort && session.address === remoteAddress;
}
server.on('message', (message, remote) => {
  const remotePort = parseInt(remote.port);
  ...
  if (Object.keys(sessions).indexOf(command[0]) === -1 && command[0] === 'give-port') {
    ...
    server.send(portMessage, 0, portMessage.length, remotePort, remoteAddress);

    var sessionMessage = new Buffer('session-is::' + newSessionId);
    server.send(sessionMessage, 0, sessionMessage.length, remotePort, remoteAddress);
  } else if(validSession(command[0], remotePort, remoteAddress)) {
    switch (command[0]) {
      case 'pong':
